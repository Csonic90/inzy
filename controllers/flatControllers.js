...
const allFlat = catchAsyncErrors(async (req, res) => {

    const resPerPage = 10;

    const flatCount = await Flat.countDocuments();

    const apiFeatures = new APIFeatures(Flat.find(), req.query)
        .search()
        .filter()

    let flat = await apiFeatures.query;
    let filteredFlatCount = flat.length;

    apiFeatures.pagination(resPerPage)
    flat = await apiFeatures.query;
...
const newFlat = catchAsyncErrors(async (req, res) => {
...
// Get Flats of current  user => /api/flat/me
const myFlat  = catchAsyncErrors(async (req, res) => {
